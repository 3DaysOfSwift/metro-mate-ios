...
| BEH-006 | Selecting a note value updates its beat count, counting display, default pattern, title, and active playback interval. | `updateNoteValue(_:)` | Unprotected | Not recorded | Pending | Pending | Pending | None |
| BEH-007 | Editing an active beat or accent updates the pattern and identifies it as a custom beat. | `toggleGridCell` and `toggleAccentCell` | Inferred | Not recorded | Pending | Pending | Pending | None |
...
| BEH-009 | Randomising creates a playable pattern, always activates and accents its first beat, retains the current BPM, and labels the result Random Beat. | `randomizeBeat()` | Unprotected | Not recorded | Pending | Pending | Pending | None |
| BEH-010 | Reset restores the Eighth pattern at 80 BPM with eight beats and the default counting mode. | `resetToBasicBeat()` | Inferred | Not recorded | Pending | Pending | Pending | None |
| BEH-011 | Saving a named preset replaces an existing preset with the same name and persists the result locally across application termination and relaunch. | `saveBeatPreset`, `persistPresets`, and README | Unprotected | Not recorded | Pending | Pending | Pending | None |
| BEH-012 | Loading a preset restores its tempo, note value, beat count, counting mode, beat pattern, accents, and name. | `loadBeatPreset(_:)` | Unprotected | Not recorded | Pending | Pending | Pending | None |
| BEH-013 | Deleting a preset removes it from persistent storage and restores the Eighth title when the deleted preset was selected. | `deleteBeatPreset(_:)` | Inferred | Not recorded | Pending | Pending | Pending | None |
